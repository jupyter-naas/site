...
export default function Home() {
  ...
  const handleScroll = () => {
    ...
    if (!docusaurusElement) return;

    const windowHeight = window.innerHeight;
    const scrollHeight = docusaurusElement.scrollHeight;
    const scrollTop = window.scrollY;

    setIsAtBottom((scrollHeight - scrollTop) < (windowHeight * 2));
  }

  useEffect(() => {
    const docusaurusElement = document.getElementsByTagName("main")[0];
    if (docusaurusElement) {
      ...
      <Link to={isAtBottom ? "/" : "#footer"}>
        <div style={{position: "fixed", bottom: "3rem", right:"3rem", zIndex: 999}}>
          <div style={{display: "flex", justifyContent: "center", alignItems: "center", "padding": "0.7rem 0.5rem", backgroundColor: "#48DD82", borderRadius: "50%"}}>
            <img style={{width: '1rem'}} src={isAtBottom ? './img/arrow_up.svg' : './img/arrow_down.svg'} alt='arrow' />
          </div>
        </div>
      </Link>
      <HomepageHeader />
